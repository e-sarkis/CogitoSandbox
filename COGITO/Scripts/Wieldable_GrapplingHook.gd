...
@export_group("GrapplingHook Settings")
...
func action_primary(_camera_collision:Vector3, _passed_item_reference : InventoryItemPD):
	...
	print("Wieldable_GrapplingHook.gd: action_primary called. Self: ", self)
...
func action_secondary(is_released:bool):
	...
	if is_released:
		...
		var tween_grappinghook = get_tree().create_tween()
		tween_grappinghook.tween_property(self,"position", default_position, .2)
	else:
		# ADS Camera Zoom IN
		var tween_cam = get_tree().create_tween()
		tween_cam.tween_property(camera,"fov", ads_fov, .2)
		var tween_grappinghook = get_tree().create_tween()
		tween_grappinghook.tween_property(self,"position", Vector3(0,default_position.y,default_position.z), .2)


# Function called when wieldable is unequipped.
func unequip():
	# TODO: Terminate ongoing grapple when unequipped
	pass
